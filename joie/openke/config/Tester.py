...
class Tester(object):
    ...
    def run_link_prediction(self, score_tail=True, type_constrain=False):
        print('in run_link_prediction')
        self.lib.initTest()
        ...
        for index, [data_head, data_tail] in enumerate(training_range):
            ...
            # else:
                # data_tail['batch_t'] = np.array([i for i in range(910)]) # this is for instype yago
                # score = self.test_one_step(data_tail)
                # self.lib.testTail(score.__array_interface__["data"][0], index, type_constrain)
